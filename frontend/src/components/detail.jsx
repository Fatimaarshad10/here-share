...
function Detail() {
  ...
  const PostDetail = async () => {
    const response = await fetch(`http://localhost:4000/post/data/${id}`, { method: "GET" });
              ...
              detail ? (
                <div class="mb-5">
                  <h2 class="mb-4">
                    ...
                    Comments</h2>
                  {
                    commentData.slice((currentPage - 1) * POSTS_PER_PAGE, currentPage * POSTS_PER_PAGE).map((data) => (
                  ...
                  <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center">
                      <li class={
                        `page-item  bg-primary text-white border-0 ${currentPage === 1 ? "disabled" : ""
                        }`
                      }></li>
                      {
                        Array.from({
                          length: Math.ceil(detail.comments.length / POSTS_PER_PAGE)
                        }).map((_, index) => (
                          <li key={index}
                            class={
                              `page-item bg-primary text-white border-0 ${currentPage === index + 1 ? "active" : ""
                              }`
                            }>
                      ...
                      <li class={
                        `page-item  bg-primary text-white border-0  ${currentPage === Math.ceil(detail.comments.length / POSTS_PER_PAGE) ? "disabled" : ""
                        }`
                      }></li>
                    </ul>
                  </nav>
